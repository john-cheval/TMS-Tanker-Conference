...
export type SpeakerDetailPropsType = SpeakersDataProps & {
  isClose?: React.Dispatch<React.SetStateAction<boolean>>;
};

const SpeakerDetailPopover = (props: SpeakerDetailPropsType) => {
  // const { isClose, company_name, image_url,image_alt_tag, description, website_link, name } =props;
  const { isClose, image_url,name,company,description,post } =props;
  return (
    <motion.div
      ...
      <div className="px-5 md:px-8 lg:px-12 2xl:px-16 2xl:pt-10 pb-6 md:pb-8 lg:pb-16">
        <div className=" ">
          ...
          <div>
            ...
            <p className="text-tms-black text-md md:text-lg font-semibold leading-3 text-center ">
              {company}
            </p>
            <p className="text-tms-black text-md md:text-lg font-semibold leading-3 text-center ">
              {post}
...
export default SpeakerDetailPopover;
